...
contract FeeToken2 is IERC20 {
  ...
  function totalSupplyFromBase() public view returns (uint256) {
  ...
  function collectFees(address account, uint256 amount) public {
    uint256 newAccountBalance = fromBase(baseBalanceOf[account]) + amount;
    // supplyBefore = baseSupply / baseScale;
    // supplyAfter = baseSupply / baseScaleAfter;
    // supplyBefore -
    //   supplyAfter = amount;
    // (baseSupply / baseScale) -
    //   (baseSupply / baseScaleAfter) = amount;
    // (baseSupply / baseScaleAfter) = (baseSupply / baseScale) - amount;
    // baseScaleAfter = baseSupply / ((baseSupply / baseScale) - amount);
    baseScale = baseSupply / ((baseSupply / baseScale) - amount);
    uint256 newBaseBalance = toBase(newAccountBalance);
    uint256 baseAmountToMint = newBaseBalance - baseBalanceOf[account];
    baseSupply += baseAmountToMint;
    baseBalanceOf[account] += baseAmountToMint;
